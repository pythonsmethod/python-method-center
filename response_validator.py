...
WELLNESS_DRIFT_PATTERNS = [
    r"(биодобавк|бад\w*)",                                       # БАД, БАДы, БАДами — all forms (GUARDRAILS-2)
    r"натуральный\s+(состав|продукт|комплекс)",
    r"(укрепля(ет|ют)|поддержива(ет|ют)|улучша(ет|ют))\s+(иммунитет|здоровье|организм)",  # plural fix (GUARDRAILS-2)
