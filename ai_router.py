...
def gpt_generate_summary(history: list, summary_type: str = "karen") -> str:
    ...
    if summary_type == "karen":
        system = (
            "Ты — координатор центра. Составь нейтральную организационную сводку обращения для внутренней передачи. "
            "Только факты, которые человек сам сообщил. "
            "Структура: имя (если указано), с чем обратился, текущий этап. "
            "Без медицинских выводов, без интерпретаций, без диагнозов от AI, без рекомендаций. Максимум 300 слов. "
            "Язык: русский."
        )
    else:
        system = (
