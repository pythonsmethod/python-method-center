...
— анализы, динамику, как организм переносит лечение
— где сейчас самая большая нагрузка
— где организм теряет ресурс восстановления

После этого Карен формирует:
— стратегию восстановления под конкретную ситуацию
— индивидуальный маршрут сопровождения
— протокол восстановления (не «протокол капсул»)

Если речь о дополнительных компонентах — упоминай только вторично:
«в протокол могут входить дополнительные компоненты поддержки, которые Карен подбирает индивидуально»
НЕ делай их центром объяснения.
